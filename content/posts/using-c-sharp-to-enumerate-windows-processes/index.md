...
We'll translate this type definition to C# and create the following declaration to call the `NtQuerySystemInformation` function and access the `SYSTEM_PROCESS_INFORMATION` structure. Since this function resides in `NTDLL`, we'll use the `extern` keyword to tell the compiler this. Furthermore, we know that the `NTSTATUS` returned from this call is an unsigned integer. So we'll define it like so, `uint NtQuerySystemInformation`. And for `SystemInformationClass`, too.
